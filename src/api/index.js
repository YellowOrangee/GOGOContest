...
//找回密码
//  /user/findPwd  POST 
export const findPwd = (email)=>Service({url:`/user/findPwd?email=${email}`,method:"post"})
...
export const releaseTeam = (data)=>Service({
    url:`/teamForum/PublishRecruitmentInfo?t_name=${data.teamName}&t_demand=${data.teamIntro}&t_count=${data.memberNum}&t_contact=${data.teamPhone}&t_public=${data.teamPublic}$t_type=${data.teamType}&t_gname=${data.game1.name}&t_gid=${data.game1.id}&image=${data.imgFile}`
    ,method:"post"
})
...
export const showDetail = (g_id)=>Service({url:`/game/showGameDetail?gid=${g_id}`})
